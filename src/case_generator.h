...
struct GeneratorConfig {
    ...
    // ================================================================================
    int U = 0;                     // 节点数量（必须为正整数）
    int N = 0;                     // 物品种类数量（必须为正整数）
    int G = 0;                     // 物品族数量（必须为正整数）
    int T = 0;                     // 时间周期数量（必须为正整数）
    bool enable_transfer = false;  // 是否启用节点间转运功能

    // ================================================================================
    // 物品-族关联关系（矩阵大小为 N x G）
    // ================================================================================
    std::vector<int> h_ig;         // 物品-族关联矩阵（扁平化存储）
                                   // h_ig[i*G + g] = 1 表示物品i属于族g
                                   // 每个物品只能属于一个族

    // ================================================================================
    // 成本参数
    // ================================================================================
    std::vector<double> cX;        // X方向生产成本向量（长度 = N）
                                   // cX[i] 表示物品i的X方向生产成本

    std::vector<double> cY;        // Y方向启动成本向量（长度 = G）
                                   // cY[g] 表示族g的启动成本

    std::vector<double> cI;        // 库存持有成本向量（长度 = N）
                                   // cI[i] 表示物品i的单位库存单位时间成本

    // ================================================================================
    // 产能占用参数
    // ================================================================================
    std::vector<double> sX;        // X方向产能占用向量（长度 = N）
                                   // sX[i] 表示生产单位物品i消耗的X方向资源

    std::vector<double> sY;        // Y方向产能占用向量（长度 = G）
                                   // sY[g] 表示启动族g消耗的Y方向资源
...
public:
     ...
     * 1. meta      - 元数据（U, N, T, enable_transfer）
